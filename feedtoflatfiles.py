...
class FileWriter(object):
    ...
    def __init__(self, directory, e_name, fields):
        self.fields = fields
        self.output_file_name = path.join(directory, e_name) + ".txt"

    def __enter__(self):
        if path.exists(self.output_file_name):
            self.output_file = open(self.output_file_name, "a").__enter__()
            return csv.DictWriter(
                self.output_file,
                fieldnames=self.fields,
                quoting=csv.QUOTE_MINIMAL)
        else:
            self.output_file = open(self.output_file_name, "w").__enter__()
            w = csv.DictWriter(
                self.output_file,
                fieldnames=self.fields,
                quoting=csv.QUOTE_MINIMAL)
    ...
    def __exit__(self, ex_type, value, traceback):
        return self.output_file.__exit__(ex_type, value, traceback)
...
def feed_to_element_files(output_directory, feed_file, element_props, version):
    with open(feed_file) as xml_doc:
        ...
        for elem in element_gen:
            with FileWriter(output_directory, elem.tag, element_props[elem.tag]) as writer:
